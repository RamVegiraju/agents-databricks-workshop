"""
AgentServer entrypoint for the Research Assistant.

Starts the MLflow AgentServer that serves the ResponsesAgent
defined in agent.py.
"""
...
import agent as agent_module  # noqa: F401
...
async def _run_lakebase_setup():
    ...
    try:
        async with AsyncDatabricksStore(
            instance_name=agent_module.LAKEBASE_INSTANCE_NAME,
            embedding_endpoint=agent_module.EMBEDDING_ENDPOINT,
            embedding_dims=agent_module.EMBEDDING_DIMS,
        ) as store:
            await store.setup()
